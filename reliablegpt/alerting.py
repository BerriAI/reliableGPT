...
class Alerting:
    def __init__(self, alert_threshold=5, send_notifications=True, user_emails=set()):
        self.unhandled_errors = {}  # dictionary of openai errors + frequency count for that hour
        ...
        if curr_time == self.current_time_block:
            ...
            if self.unhandled_errors[error_type] >= 5:
                ...
                if self.set_cooldown and time.time() - self.cooldown_start_time > 900:
                    ...
                    self.set_cooldown()
                elif self.set_cooldown is False:
                    self.send_alert(error_type, openai_error)
                    self.start_cooldown()
        else:  # reset
            self.unhandled_errors[error_type] = 1
